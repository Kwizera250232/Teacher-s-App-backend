...
async function canUsersMessage(userAId, userBId, userARole) {
...
router.get('/thread/:userId', auth, async (req, res) => {
  ...
  try {
    const allowed = await canUsersMessage(req.user.id, otherId, req.user.role);
    if (!allowed) {
      return res.status(403).json({ error: 'You can only access threads with people in your classes.' });
    }
