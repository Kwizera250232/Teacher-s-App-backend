...
router.get('/students-for-graduation', authenticateToken, requireRole('admin', 'head_teacher', 'teacher'), async (req, res) => {
  ...
  try {
    let sql = `SELECT u.id, u.name, u.email, u.school_id, s.name AS school_name, u.created_at
               FROM users u JOIN schools s ON s.id=u.school_id WHERE u.role='student' AND u.is_alumni=FALSE`;
    const params = [];
    if (school_id) { sql += ' AND u.school_id=$1'; params.push(school_id); }
    sql += ' ORDER BY u.name';
