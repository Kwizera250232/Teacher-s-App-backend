...
/**
 * Merge team quizzes (for this student) with solo quizzes.
 * Team items include is_group_quiz + group_assignment_id for the UI.
 */
function mergeStudentQuizList(soloRows, groupRows) {
  const team = dedupeGroupQuizRows(groupRows).map(groupRowToStudentQuiz);
  const teamQuizIds = new Set(team.map((t) => t.id));
  const solo = (soloRows || []).filter((q) => !teamQuizIds.has(q.id));
  const merged = [...team, ...solo];
