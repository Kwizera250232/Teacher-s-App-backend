...
function renderShareMomentHtml(moment, shareToken) {
  ...
  const preview = sharePreviewFromMoment(moment, apiBase, shareToken);
  const publicBase = frontendBase();
  const ogImage = pickOgImageUrl(moment, publicBase, shareToken);
  const photoUrls = listMomentPhotoUrls(moment, apiBase);
  const ogMime = pickFirstPhotoFilePath(moment)
    ? mimeTypeForPath(pickFirstPhotoFilePath(moment))
    : 'image/svg+xml';
  ...
  const displayImage = preview.preview_image_url || ogImage;
  const extraOgImages = photoUrls
    .slice(0, 4)
    .filter((u) => u && u !== ogImage)
...
<head>
  ...
  <meta property="og:image:type" content="${escapeHtml(ogMime)}" />
  <meta property="og:image:width" content="1200" />
  <meta property="og:image:height" content="630" />
  <meta property="og:image:alt" content="${escapeHtml(title)} — class photo preview" />
${extraOgImages}
...
function sendPreviewImageFile(moment, res) {
  const rel = pickFirstPhotoFilePath(moment);
  if (!rel) {
    return res.status(404).send('No photo in this moment.');
  }
  ...
  res.setHeader('Content-Type', mimeTypeForPath(rel));
