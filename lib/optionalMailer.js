...
async function sendMail({ to, subject, text, html, replyTo }) {
  ...
  const from = process.env.SMTP_FROM || process.env.EMAIL_FROM || 'hello@student.umunsi.com';
  ...
  if (mailtrapToken) {
    try {
      ...
      // Try bulk API first, then regular send API
      const apiUrl = process.env.MAILTRAP_BULK === 'true'
        ? 'https://bulk.api.mailtrap.io/api/send'
        : 'https://send.api.mailtrap.io/api/send';
      const fromEmail = from.replace(/^.*<(.+)>.*$/, '$1').trim();
      const fromName = (from.match(/</) && from.split('<')[0].trim()) || 'UClass';
      ...
      console.error('[mailer:mailtrap]', resp.status, errBody.slice(0, 500));
      // If bulk fails, try regular send API
      if (process.env.MAILTRAP_BULK !== 'true' && apiUrl.includes('send.api')) {
        // Already tried send API, return error
        return { sent: false, reason: `mailtrap_${resp.status}: ${errBody.slice(0, 200)}` };
      }
      // Try the other endpoint
      const fallbackUrl = apiUrl.includes('bulk') ? 'https://send.api.mailtrap.io/api/send' : 'https://bulk.api.mailtrap.io/api/send';
      const resp2 = await doFetch(fallbackUrl, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${mailtrapToken}`,
          'Content-Type': 'application/json',
      ...
      console.error('[mailer:mailtrap fallback]', resp2.status, errBody2.slice(0, 500));
      return { sent: false, reason: `mailtrap_${resp2.status}: ${errBody2.slice(0, 200)}` };
    } catch (err) {
