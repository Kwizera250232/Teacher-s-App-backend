...
const { notifyClassStudentsInApp, studentClassUrl } = require('./studentClassNotify');

function studentClassUrlLegacy(classId) {
  return studentClassUrl(classId, null);
}
...
async function notifyClassAudiencePush({
  ...
  await notifyClassStudentsInApp({
    classId: cid,
    excludeUserId: exclude,
    title,
    body: fullBody,
    contentType,
    extraPayload: { tag: pushTag },
  }).then((r) => {
    studentCount = r.students;
  });
...
module.exports = {
  ...
  notifyTeacherPush,
  studentClassUrl: studentClassUrlLegacy,
  studentClassUrlWithTab: studentClassUrl,
  teacherClassUrl,
};
