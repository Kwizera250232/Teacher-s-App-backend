...
async function resolveMailboxDomain(pool, schoolRow) {
  if (!isSchoolMailEnabled()) {
    const { persistLoginEmailDomain } = require('./schoolDomain');
    return persistLoginEmailDomain(pool, schoolRow);
  }
  await ensureSchoolMailSlug(pool, schoolRow);
  // Real mailboxes use mail.umunsi.com — do not overwrite schools.email_domain (login uses @schoolname.edu).
  return mailboxDomainForSchool(schoolRow);
}
