...
async function ensureParentHubSchema() {
  ...
  await pool.query(`
    ...
    ALTER TABLE school_announcements ADD COLUMN IF NOT EXISTS is_pinned BOOLEAN NOT NULL DEFAULT FALSE;
...
async function insertParentNotification({
  ...
  setImmediate(() => {
    try {
      ...
      }).catch((e) => console.error('[expoPush] parent notify:', e.message));
    } catch (e) {
      console.error('[expoPush] load:', e.message);
    }
    try {
      const { sendParentSms } = require('./parentSms');
      sendParentSms(parentId, { title, body, type }).catch((e) =>
        console.error('[parentSms] notify:', e.message)
      );
    } catch (e) {
      console.error('[parentSms] load:', e.message);
    }
  });
  return row;
}
