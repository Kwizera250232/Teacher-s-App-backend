...
/** Points earned specifically for a group (teacher awarded via group). */
async function groupPointTotals(classId) {
  const res = await pool.query(
    `SELECT group_id, COALESCE(SUM(value), 0)::int AS points
     FROM class_point_events
     WHERE class_id = $1 AND NOT undone AND group_id IS NOT NULL
     GROUP BY group_id`,
    [classId]
  );
  const map = {};
  for (const row of res.rows) map[row.group_id] = row.points;
  return map;
}
...
async function computeClassGroupStats(classId) {
  ...
  const groups = groupsRes.rows.map((g) => {
    ...
    const points = groupTotals[g.id] || 0;
...
module.exports = { computeClassGroupStats, studentPointTotals, groupPointTotals };
