...
/** One-time cleanup: all schools stored with @*.mail.umunsi.com → @schoolname.edu */
async function migrateSchoolLoginDomains(pool) {
  const rows = await pool.query(
    `SELECT id, name, email_domain FROM schools
     WHERE email_domain ILIKE '%mail.umunsi.com%' OR email_domain ILIKE '%.mail.%'`
  );
  for (const row of rows.rows) {
    await persistLoginEmailDomain(pool, row);
  }
}
