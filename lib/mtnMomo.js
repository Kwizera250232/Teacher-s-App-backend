...
// MTN sandbox only accepts EUR; Rwanda production accepts RWF.
// Override with MTN_CURRENCY env var if needed.
function getCurrency() {
  if (process.env.MTN_CURRENCY) return process.env.MTN_CURRENCY;
  const env = process.env.MTN_TARGET_ENV || 'sandbox';
  return (env === 'mtnrwanda' || env === 'production') ? 'RWF' : 'EUR';
}
...
// MTN production rejects non-ASCII chars in payerMessage/payeeNote (USSD charset)
...
async function requestToPay({ phone, amount, payerMessage, payeeNote, referenceId, externalId }) {
  const cfg = getConfig();
  ...
  const res = await fetch(`${getBase()}/collection/v1_0/requesttopay`, {
    ...
    body: JSON.stringify({
      ...
      payerMessage: asciiSafe(payerMessage || 'UClass subscription'),
      payeeNote: asciiSafe(payeeNote || 'UClass class access'),
    }),
  });
...
async function transfer({ phone, amount, payeeNote, referenceId, externalId }) {
  ...
  const res = await fetch(`${getBase()}/disbursement/v1_0/transfer`, {
    ...
    body: JSON.stringify({
      ...
      payeeNote: asciiSafe(payeeNote || 'UClass earnings withdrawal'),
    }),
  });
