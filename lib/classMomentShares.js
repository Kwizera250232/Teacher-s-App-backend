...
const { pickOgImageUrl, listMomentPhotoUrls } = require('./classMomentMediaUrl');

function sharePreviewFromMoment(moment, baseUrl, shareToken = null) {
  const photoUrls = listMomentPhotoUrls(moment, baseUrl);
  const imageUrl = pickOgImageUrl(moment, baseUrl, shareToken);
  ...
  return {
    ...
    preview_image_url: photoUrls[0] || imageUrl,
