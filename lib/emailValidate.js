...
function shouldSkipMailboxCheck(email, { role = null, skipMailbox = false } = {}) {
...
async function validateEmailForSignup(
  email,
  { schoolDomain, strict = false, role = null, skipMailbox = false } = {}
) {
  const allowed =
  ...
  if (shouldSkipMailboxCheck(email, { role, skipMailbox })) {
    return {
      valid: true,
      type: allowed.type,
      mailbox: {
