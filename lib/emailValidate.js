...
async function validateEmailForSignup(email, { schoolDomain, strict = false, role = null } = {}) {
  const allowed =
    role === 'parent' ? isAllowedParentEmail(email) : isAllowedSignupEmail(email, schoolDomain);
  if (!allowed.ok) {
    return { valid: false, ...allowed };
  }

  // SMTP RCPT checks are unreliable for consumer mail (Gmail, Yahoo) and block parent invites.
  if (role === 'parent') {
    return {
      valid: true,
      type: allowed.type,
      mailbox: { status: 'skipped', message: 'Format check only for parent signup.' },
    };
  }
