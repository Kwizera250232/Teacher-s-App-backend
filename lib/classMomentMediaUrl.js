...
function sharePreviewImageUrl(shareToken, apiBase = apiPublicBase()) {
  const token = String(shareToken || '').trim();
  if (!token) return null;
  return `${apiBase.replace(/\/$/, '')}/share/moment/${encodeURIComponent(token)}/preview.jpg`;
}

function pickOgImageUrl(moment, apiBase = apiPublicBase(), shareToken = null) {
  if (shareToken) {
    const proxy = sharePreviewImageUrl(shareToken, apiBase);
    if (pickFirstPhotoFilePath(moment)) return proxy;
  }
  const fp = pickFirstPhotoFilePath(moment);
  if (fp) return absoluteUploadUrl(fp, apiBase);
