...
function momentPhotosMiddleware() {
  ...
  const upload = multer({
    storage,
    limits: { fileSize: MAX_UPLOAD_SIZE, files: 10, fields: 10 },
  }).array('photos', 10);
    ...
    upload(req, res, (err) => {
      if (err) {
        if (err.code === 'LIMIT_FILE_SIZE') {
          return res.status(413).json({ error: 'A photo could not be uploaded. Try again or use fewer images in one post.' });
        }
